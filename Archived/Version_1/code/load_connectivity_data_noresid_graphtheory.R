load_connectivity_data_noresid_graphtheory <- function(corthresh){
  ...
  # igraph::graph_from_adjacency_matrix(cm[[1]])
  # 
  # ig <- graph_from_adjacency_matrix(abs(cm[[1]]>.4), diag=FALSE)
  # 
  # plot(ig)
  # 
  
  tmp <- do.call(rbind, lapply(cm, function(x) rowSums(abs(x)>.3)))
  
  colnames(tmp) <- paste0(colnames(tmp), '_Node_Degree')
